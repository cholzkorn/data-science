...
# you to work with large numbers of models with ease:
  ...
  # 2. Using list-columns to store arbitrary data structures in a data
  # frame. For example, this will allow you to have a column that
  # contains linear models.

  # 3. Using the broom package, by David Robinson, to turn models
...
# we're going to create a new variable in the by_country
...
# metrics. If we apply it to a model, we get a data frame with a single row:
...
# Now that you've seen a basic workflow for managing many models,
# let's dive back into some of the details. In this section, we'll explore
# the list-column data structure in a little more detail. It's only recently
...
# been legitimate to use a list as a column of a data frame. However,
...
# result doesn't print particularly well

data.frame(
  x = I(list(1:3, 3:5)),
  y = c("1, 2", "3, 4, 5"))

# Tibble alleviates this problem by being lazier (tibble() doesn't
# modify its inputs) and by providing a better print method:

tibble(
  x = list(1:3, 3:5),
  y = c("1, 2", "3, 4, 5"))

# It's even easier with tribble() as it can automatically work out that
# you need a list:
...
# List-columns are often most useful as an intermediate data struc-
# ture. They're hard to work with directly, because most R functions
# work with atomic vectors or data frames, but the advantage of keep-
# ing related items together in a data frame is worth a little hassle.
# Generally there are three parts of an effective list-column pipeline:

    # 1. You create the list-column using one of nest(), summarize() +
    # list(), or mutate() + a map function, as described in "Creat-
    # ing List-Columns" on page 411.
    
    # 2. You create other intermediate list-columns by transforming
    # existing list columns with map(), map2(), or pmap(). For exam-
    # ple, in the previous case study, we created a list-column of mod-
    # els by transforming a list-column of data frames.
    
    # 3. You simplify the list-column back down to a data frame or
    # atomic vector, as described in "Simplifying List-Columns" on
    # page 416.
...
# Typically, you won't create list-columns with tibble(). Instead,
# you'll create them from regular columns, using one of three methods:

  # 1. With tidyr::nest() to convert a grouped data frame into a
...
# Alternatively, you might create them from a named list, using tib
# ble::enframe().

# Generally, when creating list-columns, you should make sure they're
# homogeneous: each element should contain the same type of thing.
# There are no checks to make sure this is true, but if you use purrr
# and remember what you've learned about type-stable functions, you
# should find it happens naturally.
...
# nest() creates a nested data frame, which is a data frame with a list
# column of data frames. In a nested data frame each row is a meta
# observation: the other columns give variables that define the
# observation (like country and continent earlier), and the list-column
# of data frames gives the individual observations that make up the
# meta-observation.

# There are two ways to use nest(). So far you've seen how to use it
# with a grouped data frame. When applied to a grouped data frame,
# nest() keeps the grouping columns as is, and bundles everything
...
# You can also use it on an ungrouped data frame, specifying which
# columns you want to nest:
...
# example, in Chapter 11 you learned about stringr::str_split(),
...
# tidyr:separate_rows(), which is a wrapper around this common
# pattern).

# Another example of this pattern is using the map(), map2(), pmap()
# functions from purrr. For example, we could take the final example
# from "Invoking Different Functions" on page 334 and rewrite it to
# use mutate():
...
# Note that technically sim isn't homogeneous because it contains
# both double and integer vectors. However, this is unlikely to cause
# many problems since integers and doubles are both numeric vectors.
...
# One restriction of summarize() is that it only works with summary
# functions that return a single value. That means that you can't use it
# with functions like quantile() that return a vector of arbitrary length:
...
# summarize(), because each summary is now a list (a vector) of
# length 1:
...
# Data frames with list-columns provide a solution to a common
# problem: what do you do if you want to iterate over both the con-
# tents of a list and its elements? Instead of trying to jam everything
# into one object, it's often easier to make a data frame: one column
# can contain the elements, and one column can contain the list.

# An easy way to create such a data frame from a list is
# tibble::enframe():
...
# The advantage of this structure is that it generalizes in a straightfor-
# ward way-names are useful if you have a character vector of meta-
# data, but don't help if you have other types of data, or multiple vectors.

# Now if you want to iterate over names and values in parallel, you
# can use map2():
...
# To apply the techniques of data manipulation and visualization
# you've learned in this book, you'll need to simplify the list-column
# back to a regular column (an atomic vector), or set of columns. The
# technique you'll use to collapse back down to a simpler structure
# depends on whether you want a single value per element, or multiple values:

  # If you want a single value, use mutate() with map_lgl(),
  # map_int(), map_dbl(), and map_chr() to create an atomic vector

  # If you want many values, use unnest() to convert list-columns
...
# If you can reduce your list column to an atomic vector then it will be
# a regular column. For example, you can always summarize an object
# with it's type and length, so this code will work regardless of what
# sort of list-column you have:
...
# parts that aren't working for you.

# Don't forget about the map_*() shortcuts-you can use map_chr(x,
# "apple") to extract the string stored in apple for each element of x.

# This is useful for pulling apart nested lists into regular columns. Use
# the .null argument to provide a value to use if the element is miss-
# ing (instead of returning NULL):
...
# unnest() works by repeating the regular columns once for each ele-
# ment of the list-column. For example, in the following very simple
# example we repeat the first row four times (because there the first
# element of y has length four), and the second row once:

tibble(x = 1:2, y = list(1:4, 1)) %>%
  unnest(y)

# This means that you can't simultaneously unnest two columns that
...
# The same principle applies when unnesting list-columns of data
# frames. You can unnest multiple list-columns as long as all the data
# frames in each row have the same number of rows.
...
# into tidy data frames:
  ...
  # broom::glance(model) returns a row for each model. Each col-
  # umn gives a model summary: either a measure of model quality,
  # or complexity, or a combination of the two.

  # broom::tidy(model) returns a row for each coefficient in the
  # model. Each column gives information about the estimate or its
  # variability.

  # broom::augment(model, data) returns a row for each row in
  # data, adding extra values like residuals, and influence statistics.

# Broom works with a wide variety of models produced by the most
# popular modelling packages. See https://github.com/tidyverse/broom
